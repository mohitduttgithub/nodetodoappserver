...
export const forgetPassword = async (req, res) => {
  try {
    ...
    user.resetPasswordOptExpirey = Date.now() + 10 * 60 * 1000;
...
export const resetPassword = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ resetPasswordOtp: otp, resetPasswordOptExpirey: {$gt: Date.now()} }).select("+password")
    ...
    user.resetPasswordOptExpirey = null;
